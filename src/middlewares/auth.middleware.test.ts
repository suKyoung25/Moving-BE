...
describe("프론트의 요청을 검증한다 ", () => {
  ...
  test("validateReq, 유효한 요청이면 next()를 호출한다", () => {
  ...
  test("validateReq, 잘못된 요청이면 BadRequestError와 에러 메세지를 next()로 전달한다", () => {
...
describe("(기사) 회원가입 시 이메일과 전화번호를 DB와 대조한다", () => {
  ...
  test("이메일, 전화번호가 중복이 아니면 에러 없이 next가 호출된다", async () => {
  ...
  test("이메일이 중복이면 ConflictError가 next로 호출된다", async () => {
  ...
  test("전화번호가 중복이면 ConflictError가 next로 호출된다", async () => {
    mockGetMoverByEmail.mockResolvedValue(null); // 중복 이메일 존재하지 않음
  ...
  test("이메일, 전화번호가 중복이면 ConflictError가 next로 호출된다", async () => {
    mockGetMoverByEmail.mockResolvedValue({ id: "1", email: req.body.email } as any); // 중복 이메일 존재 함
    mockGetMoverByPhone.mockResolvedValue({ id: "1", email: req.body.phone } as any); // 중복 전화번호 존재 함
  ...
  test("레포지토리 호출 중 에러 발생 시 next로 에러가 전달된다", async () => {
...
describe("(기사) 로그인 시 이메일과 비밀번호를 DB와 대조한다", () => {
  ...
  test("존재하는 사용자고 비밀번호가 맞으면 next가 에러 없이 호출된다", async () => {
  ...
  test("사용자가 존재하지 않으면 ConflictError가 next로 호출된다", async () => {
  ...
  test("비밀번호가 틀리면 ConflictError가 next로 호출된다", async () => {
    mockGetMoverByEmail.mockResolvedValue({
      email: req.body!.email,
      hashedPassword: "hashed_password",
    } as any);
...
describe("토큰의 유효성에 따라 정보를 반환한다", () => {
  ...
  test("Authorization 헤더가 없을 때 next()만 호출된다", () => {
    optionalAuth(req, res, next);
  ...
  test("잘못된 토큰일 경우 next(error) 호출된다", () => {
