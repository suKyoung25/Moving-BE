...
async function getMyReviews(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        res.status(200).json({ status: 200, message: "리뷰 목록 조회 성공", data: reviews });
    } catch (error) {
        next(error);
    }
}
...
async function createReview(
    ...
    try {
        ...
        res.status(201).json({ status: 201, message: "리뷰 작성 성공", data: review });
    } catch (error) {
        next(error);
    }
}
...
async function updateReview(
    ...
    try {
        ...
        res.status(200).json({ status: 200, message: "리뷰 수정 성공", data: updated });
    } catch (error) {
        next(error);
    }
}
